...
},{"./nav":2,"./scrollSpy":3}],2:[function(require,module,exports){
'use strict';

var _jquery = require('../vendors/jquery/dist/jquery.js');
...
module.exports = function () {
    ...
    (0, _jquery2.default)('.nav-mobile').click(function () {
        //display in block
        (0, _jquery2.default)('ul.nav-list').toggleClass('nav-open').slideToggle('slow');
    });
    ...
    (0, _jquery2.default)(window).on('resize', function () {
        if ((0, _jquery2.default)(this).width() >= 768) {
            (0, _jquery2.default)('ul.nav-list').css({ "display": "inline-block" });
    ...
    // animation...
    var lastScrollTop = 0;
    var dh = (0, _jquery2.default)('div.header');
    ...
    var mn = (0, _jquery2.default)(".header"),
        ml = (0, _jquery2.default)('div.logo'),
        // dropshadow when scrolled
    mns = "main-nav-scrolled",
        //change logo size
    mnls = "main-nav-logo-scrolled",
        hdr = (0, _jquery2.default)('header').height();

    (0, _jquery2.default)(window).scroll(function () {
        // hides nav elements when page scroll
        (0, _jquery2.default)('.nav-open').css("display", "none");

        if ((0, _jquery2.default)(this).scrollTop() > hdr) {
    ...
    (0, _jquery2.default)(window).scroll(function () {
        ...
        if ((0, _jquery2.default)(this).scrollTop() > hdr && !mnls) {
            ml.addClass(mnls);
        } else {
            ml.removeClass(mnls);
        }
    ...
    var allNavLinks = (0, _jquery2.default)('a.link-nav');

    (0, _jquery2.default)(allNavLinks).click(function () {
        ...
        if (window.innerWidth <= 768) {
            ...
            (0, _jquery2.default)('ul.nav-list').css({ "display": "none" });
...
},{"../vendors/jquery/dist/jquery.js":6}],3:[function(require,module,exports){
'use strict';
...
var _TweenMax = require('../vendors/gsap/src/uncompressed/TweenMax.js');

var _TweenMax2 = _interopRequireDefault(_TweenMax);

var _ScrollMagic = require('../vendors/scrollmagic/scrollmagic/uncompressed/ScrollMagic.js');

var _ScrollMagic2 = _interopRequireDefault(_ScrollMagic);

var _ScrollToPlugin = require('../vendors/gsap/src/uncompressed/plugins/ScrollToPlugin.js');
...
controller.scrollTo(function (target) {
    ...
    _TweenMax2.default.to(window, 0.5, {
        scrollTo: {
...
},{"../vendors/gsap/src/uncompressed/TweenMax.js":4,"../vendors/gsap/src/uncompressed/plugins/ScrollToPlugin.js":5,"../vendors/jquery/dist/jquery.js":6,"../vendors/scrollmagic/scrollmagic/uncompressed/ScrollMagic.js":7}],4:[function(require,module,exports){
(function (global){
...
return v.match(_relNumExp);}}else {a=v.match(_numExp)||_colorLookup.transparent;}a[0]=Number(a[0]);a[1]=Number(a[1]);a[2]=Number(a[2]);if(a.length>3){a[3]=Number(a[3]);}}if(toHSL&&!wasHSL){r=a[0]/255;g=a[1]/255;b=a[2]/255;max=Math.max(r,g,b);min=Math.min(r,g,b);l=(max+min)/2;if(max===min){h=s=0;}else {d=max-min;s=l>0.5?d/(2-max-min):d/(max+min);h=max===r?(g-b)/d+(g<b?6:0):max===g?(b-r)/d+2:(r-g)/d+4;h*=60;}a[0]=h+0.5|0;a[1]=s*100+0.5|0;a[2]=l*100+0.5|0;}return a;},_formatColors=function _formatColors(s,toHSL){var colors=s.match(_colorExp)||[],charIndex=0,parsed=colors.length?"":s,i,color,temp;for(i=0;i<colors.length;i++){color=colors[i];temp=s.substr(charIndex,s.indexOf(color,charIndex)-charIndex);charIndex+=temp.length+color.length;color=_parseColor(color,toHSL);if(color.length===3){color.push(1);}parsed+=temp+(toHSL?"hsla("+color[0]+","+color[1]+"%,"+color[2]+"%,"+color[3]:"rgba("+color.join(","))+")";}return parsed;},_colorExp="(?:\\b(?:(?:rgb|rgba|hsl|hsla)\\(.+?\\))|\\B#(?:[0-9a-f]{3}){1,2}\\b"; //we'll dynamically build this Regular Expression to conserve file size. After building it, it will be able to find rgb(), rgba(), # (hexadecimal), and named color values like red, blue, purple, etc.
...
define(factory);}else if((typeof exports==='undefined'?'undefined':_typeof(exports))==='object'){ // CommonJS
module.exports=factory();}else { // Browser global
root.ScrollMagic=factory();}})(undefined,function(){"use strict";var ScrollMagic=function ScrollMagic(){_util.log(2,'(COMPATIBILITY NOTICE) -> As of ScrollMagic 2.0.0 you need to use \'new ScrollMagic.Controller()\' to create a new controller instance. Use \'new ScrollMagic.Scene()\' to instance a scene.');};ScrollMagic.version="2.0.5"; // TODO: temporary workaround for chrome's scroll jitter bug
...
_options.container.addEventListener("resize",onChange);_options.container.addEventListener("scroll",onChange);_options.refreshInterval=parseInt(_options.refreshInterval)||DEFAULT_OPTIONS.refreshInterval;scheduleRefresh();log(3,"added new "+NAMESPACE+" controller (v"+ScrollMagic.version+")");}; /**
...
Scene.trigger("change",{what:varname,newval:_options[varname]});Scene.trigger("shift",{reason:varname});}}}; /**
		 ...
		 * @param {boolean} [suppressEvents=false] - If true the shift event will be suppressed.
		 * @private
		 */var updateTriggerElementPosition=function updateTriggerElementPosition(suppressEvents){var elementPos=0,telem=_options.triggerElement;if(_controller&&telem){var controllerInfo=_controller.info(),containerOffset=_util.get.offset(controllerInfo.container), // container position is needed because element offset is returned in relation to document, not in relation to container.
...
containerOffset[param]-=_controller.scrollPos();}elementPos=elementOffset[param]-containerOffset[param];}var changed=elementPos!=_triggerPos;_triggerPos=elementPos;if(changed&&!suppressEvents){Scene.trigger("shift",{reason:"triggerElementPosition"});}}; /**
...
_durationUpdateMethod=val;try{val=parseFloat(_durationUpdateMethod());}catch(e){val=-1; // will cause error below
}} // val has to be float
...
e.preventDefault();_controller._setScrollPos(_controller.info("scrollPos")-((e.wheelDelta||e[_controller.info("vertical")?"wheelDeltaY":"wheelDeltaX"])/3||-e.detail*30));}}; /**
		 ...
		 *
		 * @returns {Scene} Parent object for chaining.
		 */this.setPin=function(element,settings){var defaultSettings={pushFollowers:true,spacerClass:"scrollmagic-pin-spacer"};settings=_util.extend({},defaultSettings,settings); // validate Element
...
if(_pin&&_options.duration===0&&settings.pushFollowers){log(2,"WARNING: pushFollowers =",true,"has no effect, when scene duration is 0.");}},0); // create spacer and insert
...
var SCENE_OPTIONS={defaults:{duration:0,offset:0,triggerElement:undefined,triggerHook:0.5,reverse:true,loglevel:2},validate:{offset:function offset(val){val=parseFloat(val);if(!_util.type.Number(val)){throw ["Invalid value for option \"offset\":",val];}return val;},triggerElement:function triggerElement(val){val=val||undefined;if(val){var elem=_util.get.elements(val)[0];if(elem){val=elem;}else {throw ["Element defined in option \"triggerElement\" was not found:",val];}}return val;},triggerHook:function triggerHook(val){var translate={"onCenter":0.5,"onEnter":1,"onLeave":0};if(_util.type.Number(val)){val=Math.max(0,Math.min(parseFloat(val),1)); //  make sure its betweeen 0 and 1
...
return arr;}}if(_type(selector)==='nodelist'||_type.Array(selector)){for(var i=0,ref=arr.length=selector.length;i<ref;i++){ // list of elements
